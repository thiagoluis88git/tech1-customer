...
func (a *apiFeature) iSendRequestToWithPayload(ctx context.Context, method, route string, payloadDoc *godog.DocString) (context.Context, error) {
	...
	loginUseCase := new(MockLoginCustomerUseCase)

	loginUseCase.On("Execute", req.Context(), "12345678910").
		Return(dto.Token{
			AccessToken: "TOKEN",
		}, nil)

	createPaymentHandler := handler.LoginCustomerHandler(loginUseCase)

	createPaymentHandler.ServeHTTP(recorder, req)

	var paymentResponse dto.Token
